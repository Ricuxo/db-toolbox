...
declare
	P_USER_ROLE		  dba_sys_privs.privilege%type := trim(upper('&priv_or_role'));
...
-- GET_ALL_TREE
		...
		begin
			...
			FOR j in (select rownum rn,grantee r,decode(admin_option,'YES',' granted with admin option','') a from dba_sys_privs where privilege=P_USER_ROLE) loop
				SHOW('Grantee: '||j.r||j.a,V_TABs,'--- Granted Roles ---',j.rn);
				get_all_tree(j.r,V_TABs);
			end loop;
			FOR j in (  select rownum rn,grantee||decode(ADMIN_OPTION,'YES',' granted with admin option','') a,grantee r from dba_role_privs where granted_role=P_USER_ROLE) loop
				SHOW(j.a,V_TABs,'--- Grantees ---',j.rn);
				IF user_or_role(j.r)='ROLE' then
					get_all_tree(j.r,V_TABs);
				END IF;	
			end loop;
		end get_all_tree;
