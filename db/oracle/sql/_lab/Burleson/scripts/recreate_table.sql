/*  */
create or replace procedure recreate_table as
...
--
   CURSOR col_cursor (c_own varchar2, c_tab VARCHAR2) IS select   
					...
					order by column_id;
--
   CURSOR  part_key (c_own varchar2, c_tab VARCHAR2) is select 	column_name,
							column_position
...
--
   CURSOR  part_cursor (c_own varchar2, c_tab VARCHAR2) IS select
                                                	PARTITION_NAME,         
							SUBPARTITION_COUNT,
							HIGH_VALUE,             
							HIGH_VALUE_LENGTH,      
					...
					from dba_tab_partitions
                                        where table_owner = c_own
                                             and table_name=c_tab
                                        order by partition_position;
--
   CURSOR subpart_key (c_own varchar2, c_tab VARCHAR2) IS select
						 	column_name,
							column_position
					  from dba_subpart_key_columns
                                          where trim(owner)=c_own
                                          and trim(name)=c_tab
                                          and trim(object_type)='TABLE'
                                          order by column_position;
--
   CURSOR subpart_tbsp (c_own varchar2, c_tab VARCHAR2) IS select unique tablespace_name
					   from dba_tab_subpartitions
                                           where table_name=c_tab and table_owner=c_own;
--
   CURSOR subpart_cursor (c_own varchar2,c_tab VARCHAR2, tab_part VARCHAR2) IS select 
								SUBPARTITION_NAME,
								tablespace_name
 					from dba_tab_subpartitions
                                        where table_owner=c_own
...
--
   CURSOR subpart_stor (c_own varchar2, c_tab varchar2, tab_part varchar2) IS select
								initial_extent,
								next_extent,
								pct_increase
 					from dba_tab_subpartitions
...
BEGIN
   ...
   LOOP
	...
	lv_string := 'DROP TABLE '|| lower(lv_owner)||'.'||lower(lv_table_name)||';';
	write_out(lv_table_name, lv_string);
	lv_first_rec := TRUE;
	lv_string := 'CREATE TABLE '|| lower(lv_owner)||'.'||lower(lv_table_name)||' (';
	write_out(lv_table_name, lv_string);
      lv_string := null;
	...
	if ((lv_data_type = 'NUMBER') and (lv_data_precision>0)) then
	 ...
	 if (lv_nullable = 'N') then
      ...
      nul_cnt:=0;
--
      OPEN part_key(lv_owner,lv_table_name);
...
--
      if lv_subpartition_key_count>0 THEN
       OPEN subpart_key(lv_owner,lv_table_name);
...
--  
       if lv_subpartition_count>1 then
        OPEN subpart_tbsp(lv_owner,lv_table_name);
        nul_cnt:=0;
        LOOP
...
--
      OPEN part_cursor(lv_owner,lv_table_name);
      ...
      FETCH part_cursor INTO    lv_PARTITION_NAME,
				...
				lv_part_LOGGING,
				lv_part_BUFFER_POOL;
      OPEN subpart_stor(lv_owner,lv_table_name,lv_partition_name);
...
--
        if lv_part_subpartition_count>0 THEN
         OPEN subpart_cursor(lv_owner,lv_table_name,lv_partition_name);
         sp_cnt:=0;
         LOOP
