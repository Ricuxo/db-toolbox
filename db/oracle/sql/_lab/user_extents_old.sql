...
SELECT tablespace_name,
       CASE WHEN COUNT(extent_id) <= 1024 THEN 1 ELSE 0 END AS M1024, 
       CASE WHEN COUNT(extent_id) > 1024  AND COUNT(extent_id) <= 3072 THEN 1 ELSE 0 END AS E3072,
       CASE WHEN COUNT(extent_id) > 3072 THEN 1 ELSE 0 END AS M3072
 FROM dba_extents
WHERE owner = '&&v_owner'
  AND segment_type IN ('INDEX','INDEX PARTITION','INDEX SUBPARTITION','TABLE','TABLE PARTITION','TABLE SUBPARTITION')
GROUP BY tablespace_name, segment_name) b
...
FROM dba_segments
WHERE 	segment_type IN ('INDEX','INDEX PARTITION','INDEX SUBPARTITION','TABLE','TABLE PARTITION','TABLE SUBPARTITION')
  AND   owner = '&&v_owner') a
