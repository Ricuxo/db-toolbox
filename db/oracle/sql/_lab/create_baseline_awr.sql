...
SELECT plan_hash_value, sql_id
  FROM TABLE (DBMS_SQLTUNE.select_sqlset (sqlset_name => 'CREATE_BASELINE_AWR'));
/*
set feedback on
...
to_char(spb.last_executed,'DD/MM/YYYY HH24:MI:SS') last_executed,EXECUTIONS
from dba_sql_plan_baselines spb
where upper(spb.plan_name) = upper('&&plan_name');
*/
clear breaks
PROMPT exec  DBMS_SQLTUNE.DROP_SQLSET( sqlset_name => 'CREATE_BASELINE_AWR' );
undef sql_id
