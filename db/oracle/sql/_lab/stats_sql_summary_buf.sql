...
loop
   ...
   then
      ...
      v_qtd_gets_execs := round(v_qtd_gets/v_qtd_execs);
      v_execs_ant := v_execs;
      v_disk_ant  := v_disk;
      v_gets_ant  := v_gets;
      v_rows_ant  := v_rows;
...
--      dbms_output.put_line(v_hash||'-'||lp1.SNAP_ID||'-'||to_char(lp1.SNAP_TIME,'DD/MM/YY-HH24:MI:SS')||'-'||v_qtd_snaptime||'-'||v_qtd_execs||'-'||v_qtd_gets||'-'||v_qtd_disk);
      dbms_output.put_line(to_char(lp1.SNAP_TIME,'DD/MM/YY-HH24:MI:SS')||' '||v_qtd_gets_execs||' '||v_qtd_snaptime||' '||v_qtd_gets||' '||v_qtd_execs);
   end if;
   exception
