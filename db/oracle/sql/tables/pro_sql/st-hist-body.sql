...
declare
	...
	cursor hist_stats (col_nm dba_tab_histograms.column_name%TYPE) is
	...
	select endpoint_number curr_ep, 
	 ...
	 where owner = v_owner
	   ...
	   and EXISTS (select null from dba_tab_cols 
  ...
  select count(1)
   ...
   where b.owner = v_owner
     ...
     and (exists (select 1 from dba_tab_columns
          ...
          or
          exists (select 1 from dba_tab_histograms
